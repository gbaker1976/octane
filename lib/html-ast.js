...
let quoteDelimiter = (str, idx, ast, buf, context) => {
	if ( context & ( CONTEXT_OPEN_COMMENT | CONTEXT_OPEN_TEXT ) ) {
		...
		ast.current.parameters[ast.current.parameters.length-1].value = buf.join('');
...
let tokens = {
	...
	' ': (str, idx, ast, buf, context) => {
		if ( context & ( CONTEXT_OPEN_COMMENT |
						 ...
						 CONTEXT_OPEN_PARAM_VALUE ) ) {
			buf.push(str[idx]);
		} else if ( context & ( CONTEXT_CLOSE_COMMENT | CONTEXT_OPEN_DECL_NAME | CONTEXT_OPEN_TAG_NAME ) ) {
			if ( context & CONTEXT_OPEN_DECL_NAME | CONTEXT_OPEN_TAG_NAME ) {
				ast.current.name = buf.join('');
			}
			if ( context & CONTEXT_OPEN_TAG_NAME ) {
