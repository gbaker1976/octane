...
let tokens = {
	...
	'/': (str, idx, ast, buf, context, line) => {
		if ( '*' === str[idx+1] ) {
			if ( context & constants.CONTEXT_OPEN_ATTRIBUTE_VALUE ) {
				throw new Error('Validation Error: comments are not valid within attribute values. Line: ' + line);
			}

			if ( context & constants.CONTEXT_OPEN_ATTRIBUTE_NAME ) {
				throw new Error('Validation Error: comments are not valid within attribute names. Line: ' + line);
			}

			if ( context & constants.CONTEXT_OPEN_SELECTOR ) {
				throw new Error('Validation Error: comments are not valid within selectors, Line: ' + line);
			}
...
let parser = (str, idx, ast, buf, context, line) => {
	let chr;
	let isWhitespace;
	...
	if ( tokens[chr] ) {
		context = tokens[chr]( str, idx, ast, buf, context, line );
	...
	return parser( str, ++idx, ast, buf, context, line );
};
